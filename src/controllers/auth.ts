...
const registerUser: RequestHandler = async (
  ...
  try {
    ...
    const token = jwt.sign({ email: email }, 'lequochuy@123!');
...
const loginUser: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    if (status && !user?.accountBlocked) {
      const token = jwt.sign({ userId: user._id }, 'lequochuy@123!', {
        expiresIn: "10h",
      });
...
const activateUser: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    const emailToken = jwt.sign({ userId: user._id }, 'lequochuy@123!', {
      expiresIn: "5m",
    });

    const userEmail = process.env.EMAIL_SMTP || "";
    const userPassword = process.env.EMAIL_PASSWORD_SMTP || "";
